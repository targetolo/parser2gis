...
async def parse(req: ParseRequest):
    ...
    runner_code = f"import sys; sys.argv = {json.dumps(args)}; from parser_2gis.main import main; main()"
    ...
    if not output_path.exists():
        error_msg = stderr.decode("utf-8", errors="replace")[-800:]
        raise HTTPException(status_code=500, detail=f"Ошибка парсера: {error_msg}")
